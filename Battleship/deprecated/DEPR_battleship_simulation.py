    ...
    for row in range(BOARD_SZ):
        for col in range(BOARD_SZ):
            if(USER_BOARD[row][col]=='H'):
                ...
                if(min_c == max_c):  # "OLD: if(min_c is None)" --> what was the point of this?? basically never going to be none, since hit found?
                    ...
                    # OLD: "if(min_c != max_c):" -- but already checking this now in 'if' above (min_c == max_c)
...
def assess_adj(usr_brd, hit_loc, ship_inds):
    ...
    if(max_conf_ind == -1):
        ...
        print(hit_loc)
        print(ADJ_CTR)
        print('we have slight problemo 2')
        return
...
def check_sunk(glob_brd, usr_brd, hit_r, hit_c):
    ...
    ship_type = glob_brd[hit_r][hit_c]
    horiz = HORIZONTAL[ship_ind_map[ship_type]]
    # print(ship_type, horiz)
    ...
    for alt_coord in range(0, BOARD_SZ): # just go through all 10, for simplicity of coding: negligible efficiency difference (O(1))
        if(horiz):
            if(glob_brd[hit_r][alt_coord]==ship_type and usr_brd[hit_r][alt_coord]=='' and alt_coord!=hit_c):
                return False
            elif(glob_brd[hit_r][alt_coord]==ship_type): # the ship we're looking for (with this hit) is actually here on the global board
                max_r = hit_r
                max_c = alt_coord
            ...
            if(glob_brd[alt_coord][hit_c]==ship_type and usr_brd[alt_coord][hit_c]=='' and alt_coord!=hit_r):
                return False
            elif(glob_brd[alt_coord][hit_c]==ship_type): # there IS something here
                max_r = alt_coord
                max_c = hit_c
                if(not FIRST_FOUND_FLAG):
                    min_r = alt_coord
...
def find_command_computer(glob_brd, usr_brd, move_r, move_c): # this takes in 1-INDEXED move
    ...
    move_c -= 1
    if(glob_brd[move_r][move_c]==''):
        return str(move_r) + " " + str(move_c) + " miss"

    poss_sunk = check_sunk(glob_brd, usr_brd, move_r, move_c)
    if(not poss_sunk):
        return str(move_r) + " " + str(move_c) + " hit"

    # in this case, poss_sunk has returned the tuple we need
...
def run_one_game():
    ...
    print("COMPUTER GENERATED BOARD:")
    ...
    while(not GAME_OVER):
        ...
        if(length == 7): # some ship has been sunk
            ...
            v = ship_sunk(ship_char, ship_minr, ship_minc, ship_maxr, ship_maxc)
            if(v == 1):
                # print(check_round_counts(USER_BOARD, counter))
                print("FINAL BOARD:")
                print(USER_BOARD)
                print("Total moves:", counter)
        ...
        # print(USER_BOARD)
        move = find_move() # ALREADY 1-INDEXED, AT THIS POINT ...
        print("Query:", "<"+str(move[0])+","+str(move[1])+">")

        # DEBUGGING CHECK: checking for repeated queries...
        token = 10*(move[0]-1)+(move[1]-1)
        if(token in query_set):
            return "REPEATED QUERY", None
        else:
            query_set.add(token)

        counter += 1
        command = find_command_computer(COMPUTER_BOARD, USER_BOARD, move[0], move[1])
        # print(command)
